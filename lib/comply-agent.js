...
// Falls back to SUPABASE_SERVICE_ROLE_KEY (the name every other file in this repo uses) so a
// deployment that only sets the ROLE_KEY variant doesn't silently fail Supabase auth here.
...
// Returns { state, version }. version is null when no row exists yet for
// this thread (a brand-new Comply case).
export async function loadState(thread_ts) {
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/thread_state?thread_ts=eq.${encodeURIComponent(thread_ts)}&select=state,version`,
    { headers: { apikey: SUPABASE_SERVICE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}` } }
  );
  const rows = await res.json();
  if (!Array.isArray(rows) || rows.length === 0) return { state: null, version: null };
  return { state: rows[0].state, version: rows[0].version };
}

async function insertState(thread_ts, state) {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/thread_state`, {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({ thread_ts, state, version: 1, updated_at: new Date().toISOString() }),
  });
  const rows = await res.json();
  return Array.isArray(rows) && rows.length > 0;
}

async function updateStateIfVersionMatches(thread_ts, state, expectedVersion) {
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/thread_state?thread_ts=eq.${encodeURIComponent(thread_ts)}&version=eq.${expectedVersion}`,
    {
      method: 'PATCH',
      headers: {
        apikey: SUPABASE_SERVICE_KEY,
...
// Merges `mergeFields` on top of `baseState` (the state the caller already
// has from its own loadState() call) using optimistic concurrency
// (thread_state.version), rather than blindly overwriting whatever's in the
// row. Two button clicks close together could otherwise silently clobber
// each other's section approvals: if the version has moved since the caller
// last read it, reload the row and retry once, merging onto the fresh state
// instead of the caller's now-stale copy.
export async function saveState(thread_ts, mergeFields, expectedVersion, baseState) {
  async function attempt(base, version) {
    const merged = { ...(base || {}), ...mergeFields };
    const ok = version == null
      ? await insertState(thread_ts, merged)
      : await updateStateIfVersionMatches(thread_ts, merged, version);
    return ok;
  }

  if (await attempt(baseState, expectedVersion)) return;

  const latest = await loadState(thread_ts);
  await attempt(latest.state, latest.version);
}
...
export function generateNoticePdf(state) {
  return new Promise((resolve, reject) => {
    ...
    doc.font('Helvetica').fontSize(10).text(
      'I, _________________________________, declare under penalty of perjury under the laws of the ' +
      'State of Washington that I am an adult over the age of 18, and that I served the following ' +
      'notice on the tenant(s) named below.',
      { lineGap: 2 }
    );
    doc.moveDown(1);
    ...
    doc.font('Helvetica').fontSize(10).text('Signature: _______________________________');
    doc.moveDown(0.75);
    doc.font('Helvetica').fontSize(10).text('Printed Name: _______________________________');
    doc.moveDown(0.75);
...
export async function runAgent(userMessage, conversationHistory, state) {
  ...
  const tools = [
    {
      name: 'lookup_tenant',
      description: 'Look up Financially Responsible tenants in Supabase by property name and unit number.',
      input_schema: {
        type: 'object',
        properties: {
          property_hint: { type: 'string', description: 'Property name or partial name' },
