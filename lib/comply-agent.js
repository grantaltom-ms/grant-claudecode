...
// Zero-padding is ambiguous across properties ("02" vs "2"), so try each numeric spelling.
// Non-numeric tokens are only upper-cased, since "A" and "a" are the same unit but "0A" is not.
function unitTokenSpellings(token) {
  if (!/^\d+$/.test(token)) return [token.toUpperCase()];
  const unpadded = String(parseInt(token, 10));
  return [...new Set([token, unpadded, unpadded.padStart(Math.max(token.length, 2), '0')])];
}
...
export function unitFilter(unitNumber) {
  ...
  // Two-token units, matched either as a letter directly followed by digits (B06) or as any
  // two tokens joined by an explicit separator (A 102, 01 - A, 06 - 1, 3614-101). Requiring
  // the separator in the second form keeps a plain "101" from being split into "1" + "01",
  // which would otherwise expand into variants matching unrelated units such as "11".
  const m = base.match(/^([A-Za-z]+)\s*[-\s]?\s*(\d+)$/) || base.match(/^(\w+)\s*[-\s]\s*(\w+)$/);
  if (m) {
    for (const left of unitTokenSpellings(m[1])) {
      for (const right of unitTokenSpellings(m[2])) {
        for (const sep of UNIT_SEPARATORS) variants.add(`${left}${sep}${right}`);
      }
    }
  }
...
export async function loadState(thread_ts) {
  ...
  if (!res.ok) {
    console.error('loadState failed:', res.status, rows);
    if (res.status === 401) console.error('supabase key config:', describeSupabaseKey());
    return null;
  }
...
export async function saveState(thread_ts, state) {
  ...
  // A dropped write silently resets the interview on the next turn, so surface it rather
  // than letting the bot look like it simply forgot the conversation.
  if (!res.ok) {
    console.error('saveState failed:', res.status, await res.text().catch(() => ''));
    if (res.status === 401) console.error('supabase key config:', describeSupabaseKey());
  }
}
