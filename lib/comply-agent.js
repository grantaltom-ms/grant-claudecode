...
// Authorization header invalid, which Supabase reports only as a generic "Invalid API key".
...
// and the `role` claim if it is a JWT. Enough to tell a missing key from an anon key pasted
// where the service_role key belongs, which is otherwise invisible from the 401 alone.
export function describeSupabaseKey() {
  const source = process.env.SUPABASE_SERVICE_KEY
    ? 'SUPABASE_SERVICE_KEY'
    : process.env.SUPABASE_SERVICE_ROLE_KEY
      ? 'SUPABASE_SERVICE_ROLE_KEY'
      : 'none';
  if (!SUPABASE_SERVICE_KEY) return { source, length: 0, role: null };

  let role = null;
  try {
    const payload = SUPABASE_SERVICE_KEY.split('.')[1];
    if (payload) role = JSON.parse(Buffer.from(payload, 'base64url').toString()).role ?? null;
  } catch {
    role = 'unparseable';
  }
  return { source, length: SUPABASE_SERVICE_KEY.length, role };
}
