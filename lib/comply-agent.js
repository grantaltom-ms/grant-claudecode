...
export function unitFilter(unitNumber) {
  ...
  if (m) {
    ...
    for (const num of new Set([rawNum, unpadded, padded])) {
      ...
      variants.add(`${letter}-${num}`);
      variants.add(`${letter} - ${num}`);
      variants.add(`${letter.toLowerCase()}${num}`);
      variants.add(`${letter.toLowerCase()}-${num}`);
      variants.add(`${letter.toLowerCase()} - ${num}`);
    }
  }
...
export function addressSearchHints(propertyHint) {
  ...
  // Strip trailing ZIP, state, then a trailing city token
  let s = raw.replace(/\b\d{5}(?:-\d{4})?\b/gi, ' ').replace(/\s+/g, ' ').trim();
  push(s);
  s = s.replace(/,?\s*\b[A-Za-z]{2}\b\s*$/i, '').trim();
  push(s);
  // Drop a final capitalized word that looks like a city ("SeaTac" / "Seatac")
  s = s.replace(/,?\s+[A-Za-z][A-Za-z.'-]+$/u, '').trim();
  push(s);
