...
//
// Corroboration is deliberately loose (sender name, sender address, or a
// subject fragment appearing on that item's line) because the model rewrites
// the surrounding prose freely; it only has to be tight enough to catch a
// shifted mapping.
export function verifyDigestItemNumbers(digestText, digestItems, itemNumbers) {
  ...
  for (const itemNumber of numbers) {
    ...
    const needles = [item.sender_name, item.sender_email, item.subject?.slice(0, 25)]
      .filter(Boolean)
