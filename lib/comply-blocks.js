...
/**
 * Compose the replacement text for a message whose buttons have just been used.
 *
 * Clicking a button removes its blocks, and it is tempting to replace the text with a short
 * "approved"/"selected" confirmation. That silently destroys context: conversation history is
 * rebuilt by re-reading the thread, so the original draft or question would vanish from the
 * model's view of what it had already written. Keeping the original text and appending the
 * outcome preserves the turn while still showing that the button was used.
 */
export function formatResolvedMessage(originalText, outcome, fallback) {
  const kept = (originalText || '').replace(/APPROVE_OR_REVISE/g, '').trim();
  return kept ? `${kept}\n\n${outcome}` : fallback;
}
