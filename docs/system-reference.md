...
| `create_draft_reply` | Creates a reply draft **and posts it in the Slack thread with the approval prompt**. Automatically fetches and preserves CC recipients from original. Sends as HTML to maintain thread continuity. |
| `create_new_draft` | Creates a new outbound email to specified recipients **and posts it in the Slack thread with the approval prompt**. |
...
**The draft card (`lib/inbox-blocks.js`'s `formatDraftSummary`):**
`create_draft_reply` and `create_new_draft` post the saved draft into the Slack thread themselves — subject, To:, Cc:, body, plus any `first_time_recipients` / `mail_tip_warnings` — ending in the approval prompt `Reply *send it*, *edit it*, or *discard*.` The model is told to answer with one short sentence and neither repeat the draft nor ask the question. This is a structural fix, not cosmetics: in production the model once wrote a reply to a correspondent in prose, asked "Send it, edit it, or discard?", and never called `create_draft_reply` at all — so when Grant answered "Send it" there was no draft in Outlook to send. Keeping the prompt inside the tool's own output means it cannot appear unless a draft really was saved. The post goes through the local `slackPost` wrapper, which never throws, so a Slack failure cannot undo a draft already saved in Outlook.

**Phantom-draft guard (`runAgent`):**
A second line of defence behind that card, since a prompt rule alone cannot rule out the model fabricating the question anyway (the same lesson as the `[#N]` numbering contract below). `runAgent` records the name of every tool that completed successfully in a run — a tool that *threw* is deliberately not recorded, since a failed `create_draft_reply` means there is no draft. At `end_turn`, `isPhantomDraftApproval()` flags a reply that asks Grant to approve a send (`looksLikeEmailSendApproval`: mentions discarding *and* sending, but not "book it") when no draft-aware tool ran — `create_draft_reply`, `create_new_draft`, `get_recent_drafts`, `send_draft`, `delete_draft`, or any of the calendar draft tools, whose card wording is close enough to be worth exempting outright. On a flag, the loop injects `PHANTOM_DRAFT_CORRECTION` as a user turn and runs **exactly once** more (never a loop): the model either saves the draft properly or confirms an existing one with `get_recent_drafts`. If the second attempt is still phantom, Grant gets the model's text plus `PHANTOM_DRAFT_WARNING` — an explicit "no draft was actually saved" — rather than a silent stall he only discovers when "send it" fails.
...
- `flagNewRecipients` — advisory only, never blocks. `create_draft_reply` and `create_new_draft` check each recipient against `TRUSTED_DOMAINS` and this mailbox's saved `email_messages` history (as sender, To, or Cc); anything neither trusted nor previously seen comes back as `first_time_recipients` in the tool result and is rendered on the draft card. A strict allowlist was deliberately not used here — Grant emails new tenants/vendors as normal business, so blocking unknown recipients would break real usage.

**Pre-send mail tips (`lib/graph.js`'s `getMailTips`/`summarizeMailTips`):**
`create_draft_reply` and `create_new_draft` call Graph's `getMailTips` for the draft's recipients (out-of-office replies, full mailbox, delivery restrictions) and return any findings as `mail_tip_warnings` on the tool result, which the draft card renders alongside the draft. Advisory only — wrapped in try/catch so a failed or unsupported mail tips call (e.g. missing `MailboxSettings.Read`, or an external domain that doesn't expose mail tips) never blocks drafting.
...
- Never write a draft out in prose and ask for approval — only `create_draft_reply`/`create_new_draft` put a draft in front of Grant, and their card shows To:, CC:, and the prompt
...
**Reply buttons.** The digest message carries one `✍️ Reply #N` Block Kit button per numbered Action Required item (`lib/inbox-blocks.js`'s `extractDigestItemNumbers` + `buildDigestBlocks`; capped at 10 buttons, 5 per row). Clicking is handled by `pages/api/inbox-interactions.js` (`handleEmailInteraction`), which deliberately drafts nothing: it resolves the item and posts "✍️ Reply to #N — sender — subject / What do you want to say?" into the digest thread, and Grant's next message drives the normal typed drafting flow through `create_draft_reply` (send approval still required). Because the button rides on the top-level digest message, `payload.message.ts` is the digest thread ts that `digest_runs.slack_thread_ts` stores. A digest with no numbered items posts as plain text with no blocks.

**The `[#N]` numbering contract — and why it is verified, not assumed.** `resolve_digest_item` looks up `digest_items.item_number`, which `saveDigestItems()` writes as the 1-based position in the `filteredEmails` array — the same position the triage prompt renders as the leading `N.` on each email line. So `[#N]` in the digest text only resolves correctly if the model **reuses that input number** rather than renumbering its Action Required items 1, 2, 3. Nothing structurally enforces that: in production the model did renumber, so `✍️ Reply #5` opened a reply to a different email than the one displayed as `[#5]` (and the older typed path — "draft reply to #1" — was mis-resolving the same way, silently, before the buttons existed). Two things now hold the contract together: the NUMBERING section of the triage prompt in `digest.js` explicitly requires reusing the email's own number and warns that the resulting numbers are usually non-contiguous, and `verifyDigestItemNumbers()` corroborates every `[#N]` against the saved `digest_items` row (sender name, sender address, or a subject fragment appearing on that item's line) before any button is built. Verification is fail-closed and all-or-nothing — one uncorroborated item drops the buttons for the whole digest and logs `Digest item numbering failed verification`, since a systematic renumber corrupts every mapping, not just one. Sending Grant one click away from replying to the wrong correspondent is worse than shipping no buttons that day.

The same check also runs **at click time** in `handleEmailInteraction`, against `payload.message.text` — the digest exactly as it was posted. This is not redundant: a Slack message carries whatever button values it was posted with, so every digest posted before the numbering fix still points at the wrong rows and no build-time check can reach back and repair one. A click whose resolved row doesn't match the `[#N]` line the reader actually saw is refused with a message naming who it *would* have opened, rather than opening it.
